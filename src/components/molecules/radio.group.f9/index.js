    ...
    return (
        <Wrapper>
            ...
            <RadioGroup row={true} aria-label="gender" name="gender1" value={value} onChange={handleChange}>
                <FormControlLabel value="gid" control={<Radio error/>} label={<span>{t("arizaqoldirish.Gid")}</span>} className="first"/>
                <FormControlLabel value="translator" control={<Radio />} label={<span>{t("arizaqoldirish.Tarjimon")}</span>} />
                <div className="double_check" style={value === "translator" ? {display:"inline-block"}:{display:'none'}}>
                    <DoubleCheck   margin="10px 0 0 120px" name1={t("arizaqoldirish.sinxron")} 
                    name2={t("arizaqoldirish.izchil")} name3={t("arizaqoldirish.yozma")} justifyContent="flex-end"  />
                </div>
            </RadioGroup>
            </FormControl>
